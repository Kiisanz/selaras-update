...
class KelolaDataMaster extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('nama_pelanggan', 'ID Customer', 'required');
        ...
        $this->form_validation->set_rules('diskon_persen', 'Diskon Persen', 'required');
        $this->form_validation->set_rules('minimum_pembelian', 'Minimum Pembelian', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->protect->protect_admin();
            $data = array(
                'vouchers' => $this->mKelolaDataMaster->get_all_vouchers(),
                "pelanggan" => $this->mKelolaDataMaster->getPelanggan(),
            );
            $this->load->view('Admin/layouts/head');
            $this->load->view('Admin/layouts/header');
            $this->load->view('Admin/layouts/aside');
            ...
            $data = array(
                'id_customer' => $this->input->post('nama_pelanggan'),
                ...
                'minimum_pembelian' => $this->input->post('minimum_pembelian'),
                'tgl_mulai' => $this->input->post('tgl_mulai'),
                'tgl_berakhir' => $this->input->post('tgl_berakhir'),
            );

            $this->mKelolaDataMaster->add_voucher($data);
            $this->session->set_flashdata('success', 'Data Voucher Berhasil Ditambahkan!');
            redirect('Admin/KelolaDataMaster/add_voucher');
        }
    }
    ...
    public function get_voucher()
    {
        ...
        $vouchers = $this->mKelolaDataMaster->get_vouchers_by_customer($user_id);
        if ($vouchers) {
            echo json_encode($vouchers);
        } else {
            echo json_encode(['error' => 'No vouchers found']);
        }
    }

    public function get_voucher_by_id()
    {
        $user_id = $this->session->userdata('id');
        $data = array(
            'vouchers' => $this->mKelolaDataMaster->get_vouchers_by_customer($user_id),
    ...
    {
        $vouchers = $this->mKelolaDataMaster->delete_voucher($id);
        if ($vouchers) {
            redirect('Admin/KelolaDataMaster/add_voucher');
        } else {
            echo json_encode(['error' => 'No vouchers found']);
